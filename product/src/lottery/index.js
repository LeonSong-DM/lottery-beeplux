...
const EXCLUDED_USER_NAMES = new Set(
...
let selectedCardIndex = [],
  ...
  basicData = {
    prizes: [], //奖品信息
    users: [], //所有人员
    eligibleUsers: [], //参与抽奖人员
    excludedUsers: [], //展示但不参与抽奖
    luckyUsers: {}, //已中奖人员
    leftUsers: [] //未中奖人员
  },
  interval,
...
function isExcludedUser(user) {
  ...
  return EXCLUDED_USER_NAMES.has(String(user[1]).trim());
}

function applyExcludedUsers() {
  if (!basicData.users || basicData.users.length === 0) {
    return;
  }
  basicData.excludedUsers = basicData.users.filter(user => isExcludedUser(user));
  basicData.eligibleUsers = basicData.users.filter(
    user => !isExcludedUser(user)
  );
  if (basicData.leftUsers && basicData.leftUsers.length > 0) {
    basicData.leftUsers = basicData.leftUsers.filter(
      user => !isExcludedUser(user)
    );
  } else {
    basicData.leftUsers = basicData.eligibleUsers.slice();
  }
}

function updateLotteryDisabled(disabled) {
  allPrizesDone = disabled;
  btns.lottery.disabled = disabled;
  if (disabled) {
    btns.lottery.textContent = "抽奖已结束";
  } else if (!isLotting) {
    btns.lottery.textContent = "开始抽奖";
  }
}
...
function initCards() {
  ...
  let isBold = false,
    showTable =
      ...
      (basicData.eligibleUsers.length || basicData.users.length),
    index = 0,
  ...
  document.querySelector("#menu").addEventListener("click", function (e) {
    ...
    switch (target) {
      ...
      case "reset":
        ...
        // 重置所有数据
        currentLuckys = [];
        basicData.leftUsers = Object.assign([], basicData.eligibleUsers);
...
function shineCard() {
  ...
  setInterval(() => {
    ...
    let displayUsers = basicData.leftUsers.concat(
      basicData.excludedUsers || []
    );
    maxUser = displayUsers.length;
    if (maxUser === 0) {
      return;
    }
    for (let i = 0; i < shineCard; i++) {
      ...
      shine(cardIndex);
      changeCard(cardIndex, displayUsers[index]);
    }
  }, 500);
}
...
window.onload = function () {
  ...
  if (fullScreenBox) {
    fullScreenBox.addEventListener("click", function () {
      let doc = document;
